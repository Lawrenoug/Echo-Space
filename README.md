...
- `WeaponMount` 现在已经会根据 `idle / run / jumpstart / fall / attack / guard / parry / execute / hurt / dead` 自动切换偏移和旋转，攻击与弹反阶段会按动画进度做挥动联动
...
2. 武器动作细调：继续按不同武器类型细调 `WeaponMount` 的偏移、旋转和挥动节奏，让短剑、长刀、双手武器后续能走不同手感。
...
- 玩家武器挂点已经正式接入动作联动，`attack` 和 `parry` 会按当前动画进度推进挥动，`guard / run / jump / fall / hurt / dead / execute` 也都有独立姿态参数
