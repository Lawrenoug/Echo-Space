...
- 玩家当前具备血量、耐力、普通攻击、防御、弹反、处决、轻按低跳 / 长按高跳、短冲刺和灵魂牵引
- 短冲刺当前是纯位移原型，优先用于验证赶路节奏、闪身穿点和后续能力扩展空间
- 灵魂牵引当前仅能在灵魂世界使用，会自动锁定范围内更合适的牵引锚点
- 敌人当前具备血量、架势、受击、破绽、处决、所属世界判定，以及稳定掉落原型
- 敌人运行时位置已经和双世界静态位置刷新解耦，切换世界时不应再被 `DualWorldObject` 写回出生点
- 当前白盒关卡已经串起跳跃、战斗、拾取、加点、双世界切换、双世界机关、灵魂牵引和短冲刺验证
...
6. 中段能力测试段：新增前置灵魂牵引锚点和落点平台，方便直接验证牵引。
7. 第二批机关段：加入反向门、双状态升降台、灵魂世界可穿透平台。
8. 连续切换谜题段：要求玩家在现实 / 灵魂之间连续切换，配合平台、按钮和牵引锚点完成推进。
9. 终段收尾：完成灵魂牵引验证后切回现实，抵达最终目标点。
...
1. 白盒实跑与修关：沿着现在这条路线完整跑图，检查哪些跳跃点、按钮位置、切世界时机、冲刺距离和牵引锚点还不顺。
2. 机关细化：继续补更多可复用机关，例如双按钮组合门、世界专属落桥、可反复切换的时间差机关。
3. 探索能力扩展：在短冲刺和灵魂牵引基础上，决定下一种正式探索能力，验证 Metroidvania 式回收路线。
